...
class TestPassageDetailSerializer:
    ...
    @mock.patch(
        "passage.serializers.PassageDetailSerializer._validate_max_massa_trekken_geremd"
    )
    def test_validate(
        self,
        mocked_validate_inrichting,
        mocked_validate_voertuigcategorie,
        mocked_validate_max_massa_trekken_geremd,
    ):
        data = {"foo": "bar"}
        serializer = PassageDetailSerializer()
        ...
        mocked_validate_max_massa_trekken_geremd.assert_any_call(data)
